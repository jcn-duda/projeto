...
| **Fase 7** trilha A (VPS) | 7.1 ✅ verificado na VPS (script já faz fetch/checkout da `esm`; HEAD == `origin/esm`, 2026-09-06). 7.3 código pronto e validado em bancada Docker local. Restam 7.2/7.4 operacionais. 7.8 (Tier 5) ✅ |
...
| 7.3 | Página de configurar na internet. `basic_auth` do Caddyfile dirigido por `CONFIGURE_PAGE_PASSWORD`: o entrypoint renderiza `/etc/caddy/configure-auth.caddy` (bcrypt no boot) e o `Caddyfile` o importa; sem a env, aberto (LAN). Validado em bancada Docker local: 401 sem senha, passa com senha, `/stream` nunca desafiado, `caddy validate` verde nas duas variantes | A3 | S | acesso | VPS com `ADDON_DOMAIN` público | CÓDIGO PRONTO (2026-09-06) | na VPS, `/configure` pede senha após pôr a env no `.env` + `docker compose up -d`. Em LAN fica aberto |
