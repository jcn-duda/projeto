...
interface CookieHarvest {
  from(raw: string | null | undefined): void;
  list(): undefined;
}

interface CookieJar {
  headers(hostname: string): string | null;
  harvest(hostname: string): CookieHarvest;
}
...
async function followProtectedUrl(
  value: string,
  ...
  if (jar && cookieJar && typeof cookieJar === 'object' && cookieJar.seed) {
    for (const [host, pairs] of Object.entries(cookieJar.seed)) {
      for (const [name, cookieValue] of Object.entries(pairs || {})) {
        jar.harvest(host).from(`${name}=${cookieValue}`);
      }
    }
  }
