...
const profilePath = require.resolve('../resolvers/profiles/bludv');
delete require.cache[profilePath];
const resolver = require(profilePath);

if (require.main === module) {
  resolver.createServer().listen(Number(process.env.PORT || 8700), '0.0.0.0', () => {
    console.log(`bludv-resolver :${process.env.PORT || 8700} — torznab em /api, fonte ${resolver.siteSelector.url()} (failover: ${resolver.siteSelector.hosts().join(', ')})`);
  });
}
