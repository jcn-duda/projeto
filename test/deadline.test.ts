...
import * as deadline from '../src/utils/deadline.js';
...
test('tarefa vence e onDeadline não dispara depois', async () => {
  ...
  const result = await deadline.raceWithDeadline(
...
test('prazo vence com o valor esperado e a tarefa segue em background', async () => {
  ...
  const result = await deadline.raceWithDeadline(task, 100, () => ({ streams: [], partial: true }));
...
test('rejeição da tarefa propaga e não deixa o onDeadline disparar', async () => {
  ...
  await assert.rejects(
    deadline.raceWithDeadline(task, 300, () => {
      deadlineFired = true;
      return { partial: true };
    }),
    /falha do provider/,
...
test('passe tardio não recebe teto dinâmico de checagem', () => {
  assert.equal(deadline.remainingCheckBudget(null, 6000, 500), null);
  assert.equal(deadline.remainingCheckBudget(undefined, 6000, 500), null);
});

test('orçamento da checagem desconta o tempo consumido e a margem final', () => {
  assert.equal(deadline.remainingCheckBudget(10000, 6000, 500), 3500);
  assert.equal(deadline.remainingCheckBudget(10000, 6000), 4000);
});
