...
const STREAMS_VERSION_DISCARD_SCRIPT = [
  ...
  "insert.run('streams:v19:movie:tt-atual:{}:account:none', JSON.stringify({ streams: ['atual'] }), now + 900000);",
  'seed.close();',
  '',
  `const cache = await import(${JSON.stringify(CACHE_URL)});`,
  '',
  // TTL futuro: v7..v15 somem por serem versão morta, não por expirar.
  "assert.deepStrictEqual(cache.get('streams:v19:movie:tt-atual:{}:account:none'), { streams: ['atual'] }, 'v19 sobe do disco');",
  ...
  "const liveRows = dbVerify.prepare(\"SELECT key FROM cache WHERE key LIKE 'streams:v19:%'\").all();",
  "assert.strictEqual(liveRows.length, 1, 'linha streams:v19 preservada no disco');",
  'dbVerify.close();',
].join('\n');

test(
  'descarte de versão obsoleta no disco — streams: v7..v18 somem, v19 sobe no boot',
