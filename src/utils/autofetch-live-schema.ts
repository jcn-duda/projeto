...
export function envDefaults(): Omit<AutofetchLiveConfig, 'paused' | 'pausedSince'> {
  return {
    ...
    autoFetchRareMax: config.debrid.autoFetchRareMax,
    autoFetchRareThreshold: config.debrid.autoFetchRareThreshold,
    autoFetchRareMaxSeeders: config.debrid.autoFetchRareMaxSeeders,
...
export function schema(): AutofetchSchemaField[] {
  ...
  return [
    ...
    {
      key: 'autoFetchRareMax',
      label: 'Título raro: teto imediato',
      type: 'number',
      group: 'volume',
      min: 1,
      max: 6,
      step: 1,
      envDefault: defaults.autoFetchRareMax,
      description: 'Com poucas alternativas fracas (título raro), o limite imediato do swarm sobe para este teto (1 a 6). Abaixo do limite comum não tem efeito.',
    },
    {
      key: 'autoFetchRareThreshold',
      label: 'Título raro: limiar de candidatos',
      type: 'number',
      group: 'volume',
      min: 0,
      max: 20,
      step: 1,
      envDefault: defaults.autoFetchRareThreshold,
      description: 'Até este número de candidatos VIÁVEIS com swarm o título é considerado raro. 0 desliga o regime raro.',
    },
    {
      key: 'autoFetchRareMaxSeeders',
      label: 'Título raro: seeders do melhor',
      type: 'number',
      group: 'volume',
      min: 1,
      step: 1,
      unit: 'seeders',
      envDefault: defaults.autoFetchRareMaxSeeders,
      description: 'Só é raro se o MELHOR candidato ficar abaixo deste teto: poucos candidatos com enxame saudável terminam sozinhos.',
    },
    {
