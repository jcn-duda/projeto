...
const DLMAG_DOMINATED_LOAD_SCRIPT = [
  ...
  '}',
  "insert.run('streams:v7:movie:tt123:{}:account:none', JSON.stringify({ streams: ['s1'] }), now + 900 * 1000);",
  ...
  `const cache = require(${JSON.stringify(CACHE_MODULE)});`,
  '',
  "assert.deepStrictEqual(cache.get('streams:v7:movie:tt123:{}:account:none'), { streams: ['s1'] }, 'streams sobrevive ao reload');",
...
const STREAMS_VERSION_DISCARD_SCRIPT = [
  ...
  "insert.run('streams:v7:movie:tt-novo:{}:account:none', JSON.stringify({ streams: ['novo'] }), now + 900000);",
  'seed.close();',
  '',
  `delete require.cache[${JSON.stringify(CACHE_MODULE)}];`,
  `const cache = require(${JSON.stringify(CACHE_MODULE)});`,
  '',
  // TTL futuro nas duas: a v6 some por ser versão morta, não por expirar.
  "assert.deepStrictEqual(cache.get('streams:v7:movie:tt-novo:{}:account:none'), { streams: ['novo'] }, 'v7 sobe do disco');",
  ...
  'const dbVerify = new DatabaseSync(process.env.CACHE_DB_PATH);',
  "const staleRows = dbVerify.prepare(\"SELECT key FROM cache WHERE key LIKE 'streams:v6:%'\").all();",
  "assert.strictEqual(staleRows.length, 0, 'linha streams:v6 apagada do disco');",
  "const liveRows = dbVerify.prepare(\"SELECT key FROM cache WHERE key LIKE 'streams:v7:%'\").all();",
  "assert.strictEqual(liveRows.length, 1, 'linha streams:v7 preservada no disco');",
  'dbVerify.close();',
].join('\n');

test(
  'descarte de versão obsoleta no disco — streams: v6 some, v7 sobe no boot',
  { skip: !hasNodeSqlite && 'node:sqlite indisponível — teste requer Node 22+' },
