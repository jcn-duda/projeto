...
/** Post mínimo exigido pelo pré-filtro de título. */
export interface MatchablePost {
  title?: string | null;
}

/** Vínculo mínimo exigido pelo `buttonId`. */
export interface ButtonRef {
  url?: string | null;
}

function normalizeFilterText(s: string | null | undefined = ''): string {
...
function stripTrailingYears(tokens: string[]): string[] {
  const out = tokens.slice();
  if (out.length >= 2 && /^\d{4}$/.test(out[out.length - 1])) out.pop();
  return out;
}

function computeWantedTokens(query: string): string[] {
...
function matchesResolverQuery(post: MatchablePost, query: string): boolean {
...
function normalizeSeasonValue(
  value: RegExpMatchArray | readonly string[] | string | number | null | undefined,
): number | null {
...
function matchesSeasonSeason(
  ...
  const season = String(post.title ?? '').match(/(?:\bS(\d{1,2})\b|(\d{1,2})\s*[ªº]\s*Temporada)/i);
...
function isGenericListPost(title: string | null | undefined = ''): boolean {
...
function buttonId(link: ButtonRef | null | undefined): string {
  return createHash('sha1').update(String(link?.url || '')).digest('hex').slice(0, 10);
}

function pickButton<T extends { url: string }>(
  links: T[],
  index: number,
  hash?: string | null,
