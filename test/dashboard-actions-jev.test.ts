...
import { aiControl } from '../src/ai/index.js';
import { createTestServer } from './e2e/e2e-harness.js';

/**
 * As 4 ações do Jev (jev-pause/jev-resume/jev-drain/jev-cooldown-reset) no
 * /dashboard-action.json. Suíte própria pela catraca de linhas: o harness
 * mínimo (token + server) basta — nenhuma dessas ações toca debrid, Jackett
 * ou cache; o estado que elas mudam é o controle EFÊMERO em memória das duas
 * filas shadow (src/ai/index.ts).
 */
