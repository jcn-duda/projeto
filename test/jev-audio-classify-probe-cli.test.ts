/**
 * Probe Jev audio-classify (ETAPA 3) — prova do CLI via spawn do script
 * real, SEM REDE e SEM chave. Mesmo contrato de argumentos do probe
 * dub-lie (mesmo parser); aqui só se prova que o script certo responde
 * com o corpus/allowlist certos.
 */
...
const ROOT = repoRoot();
const PROBE = join(ROOT, 'scripts', 'jev-audio-classify-probe.mjs');
const CASES_URL = pathToFileURL(join(ROOT, 'scripts', 'jev-audio-classify-cases.mjs')).href;
...
test('CLI: --dry-run roda sem chave, sem rede e valida o payload (só post_title)', async () => {
  ...
  assert.ok(r.stdout.includes('allowlist=OK (post_title)'));
  assert.ok(!r.stdout.includes('indexer\n'), 'payload de exemplo não carrega indexer');
...
test('CLI: valor inválido ou fora da faixa reprova com exit 3', async () => {
  for (const args of [['--threshold=abc'], ['--threshold=1.5'], ['--concurrency=99']]) {
