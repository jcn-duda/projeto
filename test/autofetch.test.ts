...
// -----------------------------------------------------------------------------
// Proteção DURÁVEL dos BR no AllDebrid (`adprot:v1`) — a camada que sobrevive ao
// restart. O hold volátil (acima) morre com o processo; o registro persistido é
// o que faz o acervo BR retido na conta não ser apagado pela limpeza automática
// na busca seguinte. Só o pool BR real (`_br` + `_dubbed`, sem `_lied`) do
// AllDebrid cria a proteção; `any`/`seeds`/recusa/falha não.
// -----------------------------------------------------------------------------
...
test('adprot: pruneMissing respeita presente, graça e hold; só o ausente envelhecido sai', () => {
  ...
  try {
    ...
    // (a) Todos presentes na conta: a poda não remove nada, nem com graça zero.
    assert.equal(held.pruneMissing('alldebrid', account, [hPresent, hGrace, hHold, hFora], 0), 0);

    // (b) Ausentes mas dentro da graça (contada do acceptedAt): ficam.
    ...
    // (c) Ausentes com graça zero: hGrace e hFora saem; o hHold, retido por
    // hold volátil vivo, fica (o status da conta pode ainda não listar o
    // download recém-aceito).
    ...
    // (d) Liberado do hold e ainda ausente: sai na passada seguinte.
...
test('adprot integrado: pools any/seeds e o BR mentiroso (_lied) não criam proteção', async () => {
  ...
  const account = accountScope(apiKey);
  const sleep = (ms: any) => new Promise((resolve) => setTimeout(resolve, ms));
  const hAny = 'd1'.repeat(20);   // dublada global (pool any)
  const hSeeds = 'd2'.repeat(20); // melhor swarm, nada dublado (pool seeds)
  const hLied = 'd3'.repeat(20);  // BR que mentiu o dublado
  const enqueued: string[] = [];
  cache.clearNamespace('adprot');
  metrics.reset();
  ...
  try {
    ...
    // Pool `any`: sem fonte BR, o fallback global dublado toca a conta — mas
    // dublagem global não vira acervo retido.
    ...
    // Pool `seeds`: nem BR nem dublado na busca, o melhor swarm entra — mas
    // melhor swarm não é acervo BR a reter.
    ...
    // BR `_lied`: nem chega à fila — o pipeline corta a release mentirosa
    // antes do autofetch (é ruído, não candidata), logo nunca cria retenção.
...
test('adprot integrado: recusa e falha do enqueue não criam proteção', async () => {
  ...
  try {
    ...
    // Recusa do serviço: o download não aconteceu, nada a reter.
    ...
    // Falha de rede: idem, e o hold volátil é liberado na hora.
