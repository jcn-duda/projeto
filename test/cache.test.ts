...
const DLMAG_DOMINATED_LOAD_SCRIPT = [
  ...
  '}',
  "insert.run('streams:v6:movie:tt123:{}:account:none', JSON.stringify({ streams: ['s1'] }), now + 900 * 1000);",
  ...
  `const cache = require(${JSON.stringify(CACHE_MODULE)});`,
  '',
  "assert.deepStrictEqual(cache.get('streams:v6:movie:tt123:{}:account:none'), { streams: ['s1'] }, 'streams sobrevive ao reload');",
...
const STREAMS_VERSION_DISCARD_SCRIPT = [
  ...
  "const insert = seed.prepare('INSERT OR REPLACE INTO cache (key, value, expires_at) VALUES (?, ?, ?)');",
  'const now = Date.now();',
  "insert.run('streams:v5:movie:tt-antigo:{}:account:none', JSON.stringify({ streams: ['antigo'] }), now + 900000);",
  "insert.run('streams:v6:movie:tt-novo:{}:account:none', JSON.stringify({ streams: ['novo'] }), now + 900000);",
  'seed.close();',
  '',
  `delete require.cache[${JSON.stringify(CACHE_MODULE)}];`,
  `const cache = require(${JSON.stringify(CACHE_MODULE)});`,
  '',
  // TTL futuro nas duas: a v5 some por ser versão morta, não por expirar.
  "assert.deepStrictEqual(cache.get('streams:v6:movie:tt-novo:{}:account:none'), { streams: ['novo'] }, 'v6 sobe do disco');",
  "assert.strictEqual(cache.get('streams:v5:movie:tt-antigo:{}:account:none'), null, 'v5 nao entra no L1');",
  '',
  // Reabre o banco: o DELETE tem que ter corrido no SQLite, não só no Map.
  'const dbVerify = new DatabaseSync(process.env.CACHE_DB_PATH);',
  "const staleRows = dbVerify.prepare(\"SELECT key FROM cache WHERE key LIKE 'streams:v5:%'\").all();",
  "assert.strictEqual(staleRows.length, 0, 'linha streams:v5 apagada do disco');",
  "const liveRows = dbVerify.prepare(\"SELECT key FROM cache WHERE key LIKE 'streams:v6:%'\").all();",
  "assert.strictEqual(liveRows.length, 1, 'linha streams:v6 preservada no disco');",
  'dbVerify.close();',
].join('\n');

test(
  'descarte de versão obsoleta no disco — streams: v5 some, v6 sobe no boot',
  { skip: !hasNodeSqlite && 'node:sqlite indisponível — teste requer Node 22+' },
