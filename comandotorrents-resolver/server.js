...
const profilePath = require.resolve('../resolvers/profiles/comandotorrents');
delete require.cache[profilePath];
const resolver = require(profilePath);

if (require.main === module) {
  resolver.createServer().listen(Number(process.env.PORT || 8701), '0.0.0.0', () => {
    console.log(`comandotorrents-resolver :${process.env.PORT || 8701} — torznab em /api, fonte ${resolver.siteSelector.url()} (failover: ${resolver.siteSelector.hosts().join(', ')})`);
  });
}
