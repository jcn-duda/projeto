...
/**
 * Addon público "Mico Leão Dublado V2" como fonte EXCLUSIVA do colhedor.
 *
 * Nunca entra no caminho da resposta: o matching dele é fraco (Coringa traz
 * Harley Quinn/South Park) e em 48 hashes de 5 obras só ~5 eram acréscimo útil.
 * Por isso o chamador (harvest-worker) junta os itens ao lote cru ANTES do
 * filtro de relevância — o lixo de outra obra morre lá, como o do Jackett.
 * Falha NUNCA derruba a colheita: devolve `[]`.
 */
...
async function search(args: SearchArgs): Promise<RawItem[]> {
  ...
  try {
    ...
    breakerFailures = 0;
    metrics.count('harvest.mico.items', out.length);
    return out;
  } catch (err: any) {
    noteFailure();
    metrics.count('harvest.mico.error');
