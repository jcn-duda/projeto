...
// A flag precisa ser fixada ANTES do import: config.js lê o process.env uma
// única vez no carregamento, e cada arquivo de teste roda em processo próprio.
process.env.JACKETT_PT_SWEEP_GLOBAL = 'false';

test('JACKETT_PT_SWEEP_GLOBAL=false desliga a varredura pt-BR nos globais', async () => {
...
test('filme e série usam título pt base na varredura global', async () => {
  const { ptSweepQueryFor } = await import('../src/providers/search-plan.js');
