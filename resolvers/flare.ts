...
/** Sessão Cloudflare memorizada por hostname. */
...
/** Mapa de sessões com TTL, injetável no fetcher. */
export interface FlareSessions {
  get(hostname: string): FlareSession | null;
  set(hostname: string, session: FlareSession): void;
  clear(): void;
  sessions: Map<string, FlareSession>;
}

/** Cookie bruto devolvido pelo FlareSolverr. */
export interface FlareCookie {
  name: string;
  value: string;
}

function createFlareSessions(): FlareSessions {
  const sessions = new Map<string, FlareSession>();
  return {
    get(hostname: string): FlareSession | null {
      const hit = sessions.get(hostname);
      return hit && hit.expiresAt > Date.now() ? hit : null;
    },
    set(hostname: string, session: FlareSession) { sessions.set(hostname, session); },
 ...
 */
function buildCookies(cookies: FlareCookie[] | null | undefined): string {
  return (cookies || [])
...
/** Opções da fábrica; os defaults de env só valem sem opção explícita. */
export interface FlareFetcherOptions {
  solverUrl?: string;
  timeoutMs?: number;
  sessionTtlMs?: number;
  userAgent?: string;
  sessions?: FlareSessions;
}
 ...
 */
function createFlareFetcher(options: FlareFetcherOptions = {}) {
  const {
  ...
  function getFlareSession(hostname: string): FlareSession | null {
    const hit = sessions.get(hostname);
  ...
  // `referer` é aceito pelos perfis (segundo argumento histórico) e ignorado.
  async function fetchTextViaFlare(url: string | URL, referer?: string): Promise<string> {
  ...
  function buildFlareHeaders(url: string | URL, referer?: string): Record<string, string> {
    ...
    // Caller omisso NÃO vira User-Agent vazio: a chave só entra quando existe
    // UA (da sessão ou do parâmetro), preservando a semântica JS original.
    const ua = session?.userAgent || userAgent;
    return {
      ...(ua ? { 'User-Agent': ua } : {}),
