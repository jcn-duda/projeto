...
const profilePath = require.resolve('../resolvers/profiles/nerdfilmes');
delete require.cache[profilePath];
const resolver = require(profilePath);

if (require.main === module) {
  resolver.createServer().listen(Number(process.env.PORT || 8702), '0.0.0.0', () => {
    console.log(`nerdfilmes-resolver :${process.env.PORT || 8702} — torznab em /api, fonte ${resolver.siteSelector.url()} (failover: ${resolver.siteSelector.hosts().join(', ')})`);
  });
}
