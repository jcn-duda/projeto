...
| **Fase 7** trilha A (VPS) | 7.1 ✅ verificado na VPS (script já faz fetch/checkout da `esm`; HEAD == `origin/esm`, 2026-09-06). 7.3 ✅ DONE — verificado em produção pelo domínio público (`/configure` e `/defaults.json` 401 com `Basic realm`, `/manifest.json` 200, 2026-09-06). Restam 7.2/7.4 operacionais. 7.8 (Tier 5) ✅ |
...
| 7.3 | Página de configurar na internet. `basic_auth` do Caddyfile dirigido por `CONFIGURE_PAGE_PASSWORD`: o entrypoint renderiza `/etc/caddy/configure-auth.caddy` (bcrypt no boot) e o `Caddyfile` o importa; sem a env, aberto (LAN). Validado em bancada Docker local: 401 sem senha, passa com senha, `/stream` nunca desafiado, `caddy validate` verde nas duas variantes | A3 | S | acesso | VPS com `ADDON_DOMAIN` público | DONE (2026-09-06) | Verificado em produção: deploy `9160cc8` healthy, `/configure` e `/defaults.json` → 401 `Basic realm="restricted"`, `/manifest.json` → 200 pelo domínio público. Detalhe operacional: o `.env` da VPS já tinha a variável preenchida (era órfã), então a página fechou com essa senha no deploy |
