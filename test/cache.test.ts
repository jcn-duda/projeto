...
const DLMAG_DOMINATED_LOAD_SCRIPT = [
  ...
  '}',
  "insert.run('streams:v8:movie:tt123:{}:account:none', JSON.stringify({ streams: ['s1'] }), now + 900 * 1000);",
  ...
  `const cache = require(${JSON.stringify(CACHE_MODULE)});`,
  '',
  "assert.deepStrictEqual(cache.get('streams:v8:movie:tt123:{}:account:none'), { streams: ['s1'] }, 'streams sobrevive ao reload');",
...
const STREAMS_VERSION_DISCARD_SCRIPT = [
  ...
  "const insert = seed.prepare('INSERT OR REPLACE INTO cache (key, value, expires_at) VALUES (?, ?, ?)');",
  'const now = Date.now();',
  "insert.run('streams:v6:movie:tt-antigo:{}:account:none', JSON.stringify({ streams: ['antigo'] }), now + 900000);",
  "insert.run('streams:v7:movie:tt-meio:{}:account:none', JSON.stringify({ streams: ['meio'] }), now + 900000);",
  "insert.run('streams:v8:movie:tt-novo:{}:account:none', JSON.stringify({ streams: ['novo'] }), now + 900000);",
  'seed.close();',
  '',
  `delete require.cache[${JSON.stringify(CACHE_MODULE)}];`,
  `const cache = require(${JSON.stringify(CACHE_MODULE)});`,
  '',
  // TTL futuro nas três: v6/v7 somem por serem versão morta, não por expirar.
  "assert.deepStrictEqual(cache.get('streams:v8:movie:tt-novo:{}:account:none'), { streams: ['novo'] }, 'v8 sobe do disco');",
  "assert.strictEqual(cache.get('streams:v7:movie:tt-meio:{}:account:none'), null, 'v7 nao entra no L1');",
  "assert.strictEqual(cache.get('streams:v6:movie:tt-antigo:{}:account:none'), null, 'v6 nao entra no L1');",
  '',
  // Reabre o banco: o DELETE tem que ter corrido no SQLite, não só no Map.
  'const dbVerify = new DatabaseSync(process.env.CACHE_DB_PATH);',
  "const staleRows = dbVerify.prepare(\"SELECT key FROM cache WHERE key LIKE 'streams:v6:%' OR key LIKE 'streams:v7:%'\").all();",
  "assert.strictEqual(staleRows.length, 0, 'linhas streams:v6/v7 apagadas do disco');",
  "const liveRows = dbVerify.prepare(\"SELECT key FROM cache WHERE key LIKE 'streams:v8:%'\").all();",
  "assert.strictEqual(liveRows.length, 1, 'linha streams:v8 preservada no disco');",
  'dbVerify.close();',
].join('\n');

test(
  'descarte de versão obsoleta no disco — streams: v6/v7 somem, v8 sobe no boot',
  { skip: !hasNodeSqlite && 'node:sqlite indisponível — teste requer Node 22+' },
