...
import type { UnwrapResolverSeed, UnwrappedResolverUrl } from './nested-url.js';
...
/** Assinatura de resposta injetada (`reply` de http-server ou do perfil). */
export type ResolverReply = (response: ServerResponse, status: number, body: string, type?: string) => unknown;

/** Handler de rota; devolve o que a rota produzir (o despacho ignora). */
export type ResolverRouteHandler = (url: URL, response: ServerResponse) => unknown;

/**
 * Semente de unwrap do /resolve. Usa o contrato real do nested-url (index
 * signature) porque os profiles expõem o `unwrapResolverUrl` da factory com
 * essa forma; um interface fechada com i/h/n não lhe é atribuível por
 * contravariância.
 */
export type UnwrapSeed = UnwrapResolverSeed;

export interface ResolverRouterOptions {
  reply: ResolverReply;
  routes: Record<string, ResolverRouteHandler>;
}
 ...
 */
function createResolverRouter({ reply, routes }: ResolverRouterOptions) {
  return async function handleRequest(request: IncomingMessage, response: ServerResponse): Promise<unknown> {
...
function createHealthRoute({ reply }: { reply: ResolverReply }): ResolverRouteHandler {
  return function handleHealth(url: URL, response: ServerResponse) {
...
function createSearchRoute({ reply, search, renderHtml }: {
  ...
  // `any[]` é a fronteira do factory: cada perfil tipa os itens do seu jeito
  // (ResolverPost, RedeWork, …) e a contravariância de parâmetro rejeitaria o
  // renderer tipado se a assinatura fosse `unknown[]`.
  renderHtml: (items: any[]) => string;
}): ResolverRouteHandler {
  return async function handleSearch(url: URL, response: ServerResponse) {
...
function createResolveRoute({
  ...
  resolveButton: (url: string, index: number, hash: string | null, count: string | null) => Promise<string>;
  fields?: { index: string; hash: string; count: string };
  validateIndex?: boolean;
}): ResolverRouteHandler {
  return async function handleResolve(url: URL, response: ServerResponse) {
...
function createDlRoute({ reply, resolveButton }: {
  ...
  return async function handleDl(url: URL, response: ServerResponse) {
...
function createApiRoute({ reply, capsXml, emptyXml, renderXml, search }: {
  reply: ResolverReply;
  capsXml: () => string;
  emptyXml: (category: number, type: string) => string;
  // Mesma fronteira do createSearchRoute: o feed itera itens tipados por perfil.
  renderXml: (items: any[], category: number) => string;
  search: (q: string) => Promise<unknown[]>;
}): ResolverRouteHandler {
  return async function handleApi(url: URL, response: ServerResponse) {
