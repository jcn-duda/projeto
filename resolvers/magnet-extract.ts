...
function decodeEncodedValue(val: string, decodeEntities: DecodeEntities): string | null {
...
/** Configuração da factory de extractMagnet. */
export interface MagnetExtractorOptions {
  decodeEntities: DecodeEntities;
  encodedVariants?: boolean;
  b64DataLink?: boolean | null;
}

/** Configuração do bloco genérico de descoberta do próximo salto. */
export interface DiscoverNextUrlOptions {
  isProtectorHost(hostname: string): boolean;
  decodeEntities: DecodeEntities;
  protectorSuffixes: string[];
  jsVarPattern?: RegExp | null;
}
 ...
 * decodificado.
 */
function createMagnetExtractor({ decodeEntities, encodedVariants = false, b64DataLink = null }: MagnetExtractorOptions) {
  ...
  return function extractMagnet(html: string | null | undefined): string | null {
    if (!html) return null;
...
function discoverNextUrl(
  ...
  { isProtectorHost, decodeEntities, protectorSuffixes, jsVarPattern = null }: DiscoverNextUrlOptions,
): string | null {
